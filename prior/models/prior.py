...
class Prior(PretrainModel):
    task='pretrain'
    def __init__(self, text_encoder, image_encoder, gpus,  stage1_epochs=20, stage2_epochs=30, stage3_epochs=50, max_epochs=100, stage1_warmup_epochs=1, stage2_warmup_epochs=1, stage3_warmup_epochs=5, batch_size=16, optim='adam', scheduler='linear_warmup_cosine_annealing', stage1_learning_rate=1e-5, stage1_learning_rate_start=1e-7, stage1_learning_rate_end=0, stage1_weight_decay=1e-6,  stage2_learning_rate=1e-5, stage2_learning_rate_start=1e-7, stage2_learning_rate_end=0, stage2_weight_decay=1e-6, stage3_learning_rate=5e-6, stage3_learning_rate_start=1e-8, stage3_learning_rate_end=0, stage3_weight_decay=1e-6, temperature=0.1, local_temperature=0.1, embed_dim=512, image_rec_drop_out_rate=0.5, spb_k=512, num_queries=16, gahter_pool='avg', lambda_proto=10, exclude_bn_bias = False, train_dataset=None, validation_dataset=None, num_workers=0, temp_decay='fixed', frozen_text_encoder=False, ckpt_path='checkpoints/'):
    ...
    def stage1_step(self, batch, stage='stage1'):
        '''
        Stage 1 only employs the alignment without SPB, in order to speed up the convergence and avoid unstable training
        '''
        image = batch['image']
        text = batch['text']
        '''
